...
+ The site has been hosted as a subdomain on AWS EC2 using https://melpop.iklem.com/
+ Node.js has been used as the web server, however the static files (css and images) are served from Nginx
+ PM2 manages the Node.js process, so it will reload it if the server reboots
+ An SSL certificate has been generated by Let's Encrypt and installed with Certbot
+ Accessing the website through its IP (http://3.17.18.63) instead of the subdomain, will show my CV
+ Overall Rating A+ in SSL Report: melpop.iklem.com from [Qualys SSL Labs](https://www.ssllabs.com/ssltest/analyze.html?d=melpop.iklem.com)
...
The live demo is available on https://melpop.iklem.com/
